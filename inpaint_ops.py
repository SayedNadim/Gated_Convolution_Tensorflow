...
@add_arg_scope
def gen_snconv(x, cnum, ksize, stride=1, rate=1, name='conv', ll=False,
...
def gated_conv(x, cnum, ksize, stride=1, rate=1, name='gated_conv',
    ...
    gated_mask = tf.layers.conv2d(
        xin, cnum, ksize, stride, dilation_rate=rate,
        activation=None, padding=padding, name=name + "_mask")

    return tf.nn.leaky_relu(x) * tf.nn.sigmoid(gated_mask)
...
def spectral_normed_weight(W, u=None, num_iters=2, update_collection=None, with_sigma=False, name="sn_w"):
    # Usually num_iteration = 1 will be enough.
    # I updated iterations as 2. No added improvement.
    W_shape = W.shape.as_list()
    W_reshaped = tf.reshape(W, [-1, W_shape[-1]])
    if u is None:
        u = tf.get_variable(name + "_u", [1, W_shape[-1]], initializer=tf.truncated_normal_initializer(),
                            trainable=False)
...
def random_ff_mask(parts, maxVertex=10, maxLength=40, maxBrushWidth=14, maxAngle=360, im_size=(256, 256), name='fmask'):
...
########################################################################################################################
# Old code for rectangle box mask generation
########################################################################################################################
...
########################################################################################################################
# Discounting Mask - Not used in Gated Convolution
########################################################################################################################
...
########################################################################################################################
# Test Contextual Attention Module for images
########################################################################################################################
