...
def multigpu_graph_def(model, data, guides, config, gpu_id=0, loss_type='g'):
    files = None
...
if __name__ == "__main__":
    ...
    if config.VAL:
        ...
        for i in range(config.STATIC_VIEW_SIZE):
            ...
            static_images = ng.data.DataFromFNames(static_fnames, config.IMG_SHAPES, nthreads=1,
    ...
    discriminator_training_callback = ng.callbacks.SecondaryTrainer(
        ...
        graph_def_kwargs={
            'model': model, 'data': data, "guides": None, 'config': config, 'loss_type': 'd'},
    )
    ...
    trainer = ng.train.Trainer(
        ...
        graph_def_kwargs={
            'model': model, 'data': data, "guides": None, 'config': config, 'loss_type': 'g'},
